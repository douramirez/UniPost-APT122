...
export default function DashboardPage() {
  ...
  const [sortConfig, setSortConfig] = useState<SortConfig>({ key: "collectedAt", direction: "desc" });

  // --- CARGA DE PERFILES ---
  useEffect(() => {
    fetch("/api/bsky/profile").then(r => r.json()).then(d => d.ok && setBskyProfile(d.profile)).catch(console.error);
    fetch("/api/instagram/profile").then(r => r.json()).then(d => {
        if (d.ok && d.profile) setIgProfile({ ...d.profile, posts: d.profile.mediaCount || d.profile.posts || 0 });
    }).catch(console.error);
  ...
  const processedMetrics = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
        ...
        // Manejo de fechas
        if (sortConfig.key === "collectedAt") {
            valA = new Date(a.collectedAt || 0).getTime();
            valB = new Date(b.collectedAt || 0).getTime();
        }
  ...
  // Helper para flechitas de orden
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900 text-white p-8 md:p-12 lg:p-16">
      ...
      <div className="max-w-7xl mx-auto mb-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div className="relative bg-white/5 border border-white/10 p-8 rounded-3xl shadow-xl backdrop-blur-sm overflow-hidden group hover:bg-white/10 transition duration-300">
          <div className="absolute -right-6 -top-6 opacity-10 pointer-events-none group-hover:opacity-20 transition transform group-hover:scale-110">
             <Image src={BskyIcon} alt="logo" width={120} height={120} />
          </div>
          <div className="relative flex items-center gap-5 z-10">
            {bskyProfile ? (
              <>
                <div className="w-20 h-20 shrink-0 rounded-full border-2 border-sky-400 p-1">
                    <img src={bskyProfile.avatar || ""} className="w-full h-full rounded-full object-cover bg-black/20" alt="Bluesky Avatar" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs uppercase text-sky-300 font-bold tracking-wider mb-1">Bluesky</p>
                  <p className="font-bold text-xl leading-tight truncate">{bskyProfile.displayName}</p>
                  <p className="text-sm opacity-60 truncate mb-3">@{bskyProfile.handle}</p>
                  <div className="flex gap-4 text-xs font-mono bg-black/20 inline-flex px-3 py-1.5 rounded-lg">
        ...
        <div className="relative bg-white/5 border border-white/10 p-8 rounded-3xl shadow-xl backdrop-blur-sm overflow-hidden group hover:bg-white/10 transition duration-300">
          <div className="absolute -right-6 -top-6 opacity-10 pointer-events-none group-hover:opacity-20 transition transform group-hover:scale-110">
             <Image src={IgIcon} alt="logo" width={120} height={120} />
          </div>
          <div className="relative flex items-center gap-5 z-10">
            {igProfile ? (
              <>
                <div className="w-20 h-20 shrink-0 rounded-full border-2 border-pink-500 p-1">
                    <img src={igProfile.profilePictureUrl || ""} className="w-full h-full rounded-full object-cover bg-black/20" alt="IG Avatar" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs uppercase text-pink-300 font-bold tracking-wider mb-1">Instagram</p>
                  <p className="font-bold text-xl leading-tight truncate">{igProfile.name}</p>
                  <p className="text-sm opacity-60 truncate mb-3">@{igProfile.username}</p>
                  <div className="flex gap-4 text-xs font-mono bg-black/20 inline-flex px-3 py-1.5 rounded-lg">
        ...
        <div className="relative bg-white/5 border border-white/10 p-8 rounded-3xl shadow-xl backdrop-blur-sm overflow-hidden group hover:bg-white/10 transition duration-300">
          <div className="absolute -right-6 -top-6 opacity-10 pointer-events-none group-hover:opacity-20 transition transform group-hover:scale-110">
             <Image src={FbIcon} alt="logo" width={120} height={120} />
          </div>
          <div className="relative flex items-center gap-5 z-10">
            {fbProfile ? (
              <>
                <div className="w-20 h-20 shrink-0 rounded-full border-2 border-blue-500 p-1">
                    <img src={fbProfile.picture?.data?.url || ""} className="w-full h-full rounded-full object-cover bg-black/20" alt="FB Avatar" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs uppercase text-blue-300 font-bold tracking-wider mb-1">Facebook</p>
                  <p className="font-bold text-xl leading-tight truncate">{fbProfile.name}</p>
                  <div className="mt-3 flex gap-4 text-xs font-mono bg-black/20 inline-flex px-3 py-1.5 rounded-lg">
        ...
        {(["GENERAL", "BLUESKY", "INSTAGRAM", "FACEBOOK"] as TabKey[]).map((tab) => (
          <button
            ...
            className={`px-8 py-2.5 rounded-full text-sm font-bold border transition duration-300 ${
              activeTab === tab
                ? "bg-white text-indigo-900 border-white shadow-[0_0_20px_rgba(255,255,255,0.3)] scale-105"
      ...
      {/* ANALÍTICA DE HASHTAGS */}
      {hashtagStats.length > 0 && activeTab === "GENERAL" && (
        <div className="max-w-7xl mx-auto mb-16">
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-3">
                #️⃣ Desempeño de Hashtags <span className="text-xs bg-white/10 px-3 py-1 rounded-full font-normal border border-white/10">Top 5</span>
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
                {hashtagStats.slice(0, 5).map((tag, idx) => (
                    <div key={idx} className="bg-black/20 border border-white/10 p-5 rounded-2xl hover:bg-white/5 transition hover:-translate-y-1 duration-300">
                        <p className="text-indigo-300 font-bold truncate text-lg mb-3">{tag.tag}</p>
                        <div className="flex justify-between items-end text-sm border-t border-white/10 pt-3">
                            <div className="flex flex-col">
                                <span className="text-xs opacity-50 mb-1">Interacciones</span>
                                <span className="font-mono">❤️ {tag.totalLikes}</span>
                            </div>
                            <span className="text-xs bg-white/10 px-2 py-1 rounded">{tag.postsCount} posts</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="overflow-x-auto">
                ...
                <thead>
                    <tr className="bg-black/20 text-white/50 uppercase text-xs tracking-wider font-bold cursor-pointer select-none">
                        ...
                        <th className="py-5 px-8 text-center hover:text-white transition" onClick={() => handleSort("likes")}>
                            Likes <SortIcon colKey="likes" />
                        </th>
                        <th className="py-5 px-8 text-center hover:text-white transition" onClick={() => handleSort("comments")}>
                            Comentarios <SortIcon colKey="comments" />
                        </th>
                        <th className="py-5 px-8 text-right hover:text-white transition" onClick={() => handleSort("collectedAt")}>
                            Fecha <SortIcon colKey="collectedAt" />
                        </th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                    ...
                    <tr key={m.id} className="hover:bg-white/5 transition-colors group">
                        ...
                        <td className="py-5 px-8 text-right text-xs opacity-40 font-mono">
                            {m.collectedAt ? new Date(m.collectedAt).toLocaleDateString('es-CL', { day: '2-digit', month: 'short', year: 'numeric' }) : "-"}
                        </td>
                    </tr>
                    ))}
                </tbody>
            ...
            {totalPages > 1 && (
                <div className="p-6 border-t border-white/10 flex justify-center items-center gap-6 bg-black/20">
                    <button 
                        ...
                        className="text-sm px-5 py-2 rounded-lg bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:hover:bg-transparent transition font-medium border border-white/10"
                    >
                        ◀ Anterior
                    </button>
                    <span className="text-sm font-mono text-white/60 bg-black/30 px-4 py-1 rounded-full border border-white/5">
